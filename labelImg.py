...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def openPrevImg(self, _value=False):
        print("上一张图片函数开始1")
        imagLists2 = []
        imagLists1 = os.listdir(self.fpath_prevImg)
        for imagList1 in imagLists1:
            index = imagList1.rfind('.')
            if imagList1[index:] == '.jpg':
                imagLists2.append(imagList1)
        print("上一张图片函数开始1 imagLists2")
        print(imagLists2)
        ...
        print("上一张图片函数开始3")
        currIndex = 0
        for i in range(len(imagLists2)):
            if "F:\labelImg-master\image\\" + str(imagLists2[i]) == self.filePath:
                print("上一张图片函数开始")
                print(self.filePath)
                print("上一张图片函数结束")
                currIndex = i
        ...
        print("当前匹配图像是")
        print(self.filePath)
        print(self.fpath_prevImg)
        if currIndex >= 1:
            filename ="F:\labelImg-master\image\\" + str(imagLists2[currIndex - 1])
            i = 1
            while os.path.exists(filename) == False:
                filename = imagLists2[currIndex - i]
                i = i + 1
            self.loadFile(filename)
    # 下一张图片
